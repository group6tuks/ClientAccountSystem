...
function selectAccount(user){
    ...
    let stringOut="";
    let db = new sqlite3.Database('./ClientAccountsDatabase.sqlite3', sqlite3.open.read.write, (err) => {
        if (err) {
...
function selectAccountType(account){
    const sqlite3 = require('sqlite3').verbose();
    let stringOut="";
    let db = new sqlite3.Database('./ClientAccountsDatabase.sqlite3', sqlite3.open.read.write, (err) => {
        if (err) {
...
function logWithdrawalTransaction(account,amount){
    const sqlite3 = require('sqlite3').verbose();
    let stringOut;
...
function logDepositTransaction(account,amount){
    const sqlite3 = require('sqlite3').verbose();
    let stringOut;
    let currentDate = new Date(),
        day = currentDate.getDate(),
...
function selectBalance(account) {
    const sqlite3 = require('sqlite3').verbose();
    let num=0;
    let db = new sqlite3.Database('./ClientAccountsDatabase.sqlite3', sqlite3.open.read.write, (err) => {
        if (err) {
